/*
  ...
  .-_'''-.       ____     .-------.      ______          .-''-.   ,---.   .--.  ______        ___    _  .-./`)  ,---.   .--.     ,-----.     
 '_( )_   \    .'  __ `.  |  _ _   \    |    _ `''.    .'_ _   \  |    \  |  | |    _ `''.  .'   |  | | \ .-.') |    \  |  |   .'  .-,  '.   
|(_ o _)|  '  /   '  \  \ | ( ' )  |    | _ | ) _  \  / ( ` )   ' |  ,  \ |  | | _ | ) _  \ |   .'  | | / `-' \ |  ,  \ |  |  / ,-.|  \ _ \  
. (_,_)/___|  |___|  /  | |(_ o _) /    |( ''_'  ) | . (_ o _)  | |  |\_ \|  | |( ''_'  ) | .'  '_  | |  `-'`"` |  |\_ \|  | ;  \  '_ /  | : 
|  |  .-----.    _.-`   | | (_,_).' __  | . (_) `. | |  (_,_)___| |  _( )_\  | | . (_) `. | '   ( \.-.|  .---.  |  _( )_\  | |  _`,/ \ _/  | 
...
#define NAME "GardenDuino"                                                                                                                                         
#define VERSION "2.0"
#define DATE "7.2022"


//edz 1.2019
//edz 13.7.2021 : Added watchDog
//edz 3.8.2021  : replaced watchdog library / added activity on display
//edz 7.2022    : version for AXB mega256
...
#include <Watchdog.h> //https://github.com/janelia-arduino/Watchdog
Watchdog watchdog;
...
#include <menu.h>  //ArduinoMenu Library by Rui Azevedo https://github.com/neu-rah/ArduinoMenu (Version 4.21.4)
...
MENU(mainMenu, VERSION , doNothing, noEvent, wrapStyle,
   ...
   OP("test WDT",testWDT,enterEvent)
);
...
//  WatchDog::start();
  ...
  oled.begin(&Adafruit128x32, OLED_I2C_ADDRESS); //check config
 ...
 */
const unsigned long BLINK_ACTIVITY_TIME = 200;
unsigned long activity_time = 0 ;
...
void loop() {
  ...
  if (nav.sleepTask) 
  {
    ...
    if ((time - activity_time) >= BLINK_ACTIVITY_TIME)
    {
       activity_time = time;
       oled.setCursor(0,4);
...
void splatsh() 
{
   ...
   Serial.println (String(" gardenDuino   Version ")+String(VERSION)+String("   electroZ microsystems  " + String(DATE)));
}
